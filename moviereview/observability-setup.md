# Observability Setup Guide for MovieReview Service

This guide provides detailed instructions for setting up Prometheus, Grafana, and Istio to monitor the MovieReview service.
...
- MovieReview service running with observability configurations
...
5. Create a custom dashboard for MovieReview metrics:
   - Click "+" > Dashboard
   - Add panels for:
     - HTTP request rate: `rate(http_server_requests_seconds_count{application="moviereview"}[5m])`
     - HTTP request duration: `http_server_requests_seconds_sum{application="moviereview"} / http_server_requests_seconds_count{application="moviereview"}`
     - Service method invocations: `rate(service_ReviewServiceImpl_getAllReviews_invocations_total[5m])`
     - Error rate: `rate(http_server_requests_seconds_count{application="moviereview",status="5xx"}[5m])`
...
### 3.2 Enable Istio Injection for MovieReview Namespace

```bash
kubectl label namespace default istio-injection=enabled
```

### 3.3 Deploy MovieReview with Istio

Create a Kubernetes deployment for MovieReview:
  ...
  template:
    ...
    spec:
      ...
      - name: moviereview
        ...
        - name: MYSQL_URL
          value: "jdbc:mysql://mysql:3306/moviereview"
        - name: MYSQL_USERNAME
          value: "root"
        - name: MYSQL_PASSWORD
          value: "root"
...
```bash
kubectl apply -f moviereview-deployment.yaml
```
...
```bash
kubectl apply -f moviereview-gateway.yaml
```
...
2. Enter query: `http_server_requests_seconds_count{application="moviereview"}`
3. Click "Execute"
4. You should see metrics from the MovieReview service
...
1. Generate some traffic to the MovieReview service
2. Access Jaeger UI (http://localhost:16686)
3. Select "moviereview" from the Service dropdown
4. Click "Find Traces"
5. You should see traces from the MovieReview service

### 6.4 Verify Istio Integration

1. Access Kiali dashboard (http://localhost:20001)
2. Navigate to the Graph view
3. You should see the MovieReview service in the service mesh
